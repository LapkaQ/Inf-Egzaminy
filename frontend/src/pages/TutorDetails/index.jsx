      ...
      >
        ...
        <div className="bg-surface-2 border border-line rounded-xl p-5 mb-6 space-y-3">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span
              className={`font-black text-lg ${GRAD} bg-clip-text text-transparent`}
            >
              {price} zł
            </span>
          </div>
...
export const TutorDetails = () => {
  ...
  const weekHasSlots = weekDays.some(
    (d) => (slotsByDate[d.toDateString()] || []).length > 0,
  )
  ...
  const maxWeekOffset = useMemo(() => {
    ...
    const lastSlot = new Date(
      Math.max(...futureSlots.map((s) => new Date(s.start_time).getTime())),
    )
    ...
    return (
      <div className="min-h-screen bg-page text-white pt-[90px] flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/tutors" className="text-accent hover:underline">
            ← Wróć do listy
          </Link>
        </div>
      </div>
  ...
  const subjectDisplay = subjects
    .map((s) => (s.name === 'INF03' ? 'INF.03' : 'INF.04'))
    .join(' & ')
    ...
    <div className="min-h-screen bg-page text-white pt-[90px]">
      <div className="max-w-6xl mx-auto px-8">
        ...
        <div className="pt-8 pb-2">
          <Link
            to="/tutors"
            className="inline-flex items-center gap-1.5 text-subtle text-sm hover:text-white transition-colors"
          >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8 items-start">
            ...
            <div className="animate-fade-up">
              ...
              <div className="flex items-center gap-5 mb-6">
                <div
                  className={`w-[88px] h-[88px] rounded-full ${GRAD} flex items-center justify-center text-white text-[1.8rem] font-black shrink-0 border-2 border-accent/30`}
                >
                  {initials(tutorName)}
                </div>
              ...
              <div className="flex gap-2 mb-7">
                {subjects.length > 0 ? (
                  subjects.map((s) => (
                    <span
                      key={s.id}
                      className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-accent/10 border border-accent/25 rounded-full text-xs font-semibold text-violet-300 tracking-wider"
                    >
                  ...
                  <span className="text-subtle text-sm">
                    Brak przypisanych kwalifikacji.
                  </span>
                )}
              </div>
              ...
              {profile.bio && (
                <div className="bg-surface border border-line rounded-2xl p-7 mb-5">
                  ...
                  <p className="text-subtle text-sm leading-[1.8]">
                    {profile.bio}
                  </p>
                </div>
              )}
              ...
              <div className="grid grid-cols-3 gap-4 mb-8">
                {[
                  ...
                  {
                    label: 'Cena / godz.',
                    value: `${profile.price_per_hour} zł`,
                  },
                ].map((s) => (
                  <div
                    key={s.label}
                    className="bg-surface border border-line rounded-xl p-5 text-center"
                  >
                    <div
                      className={`text-[1.5rem] font-black tracking-[-0.04em] ${GRAD} bg-clip-text text-transparent`}
                    >
                      {s.value}
                    </div>
                    <div className="text-[0.78rem] text-subtle mt-1">
                      {s.label}
                    </div>
                  </div>
                ))}
              ...
              <div>
                <div className="flex items-center justify-between mb-5">
                  <h2 className="text-[1.15rem] font-black tracking-[-0.03em]">
                    Wybierz termin
                  </h2>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-subtle font-medium min-w-[140px] text-center">
                      ...
                      {formatDateShort(weekDays[6])}
                    </span>
                    <button
                      onClick={() =>
                        setWeekOffset(Math.min(maxWeekOffset, weekOffset + 1))
                      }
                ...
                <div className="bg-surface border border-line rounded-2xl overflow-hidden">
                  ...
                  <div className="grid grid-cols-7 border-b border-line">
                    {weekDays.map((day, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className={`text-center py-3 ${i < 6 ? 'border-r border-line' : ''} ${isPast ? 'opacity-40' : ''}`}
                        >
                          <div className="text-[0.68rem] font-semibold text-subtle uppercase tracking-wider">
                            {DAYS_SHORT[day.getDay()]}
                          </div>
                          <div
                            className={`text-sm font-bold mt-0.5 ${isToday ? 'text-accent' : 'text-white'}`}
                          >
                            {day.getDate()}
                          </div>
                          {daySlots.length > 0 && (
                  ...
                  <div className="grid grid-cols-7 min-h-[200px]">
                    {weekDays.map((day, i) => {
                      const isPast =
                        day < now && day.toDateString() !== now.toDateString()
                      const daySlots = (slotsByDate[day.toDateString()] || [])
                      ...
                      return (
                        <div
                          key={i}
                          className={`flex flex-col gap-1.5 p-2 ${i < 6 ? 'border-r border-line' : ''} ${isPast ? 'opacity-30 pointer-events-none' : ''}`}
                        >
                          {daySlots.length > 0 ? (
                            daySlots.map((slot) => (
                              <button
                              ...
                              `}
                              >
                                <div className="text-[0.72rem] font-bold text-white">
            ...
            <div className="sticky top-[90px] animate-fade-up [animation-delay:0.15s]">
              <div className="bg-surface border border-line rounded-2xl p-8">
                <div className="text-[1.8rem] font-black tracking-[-0.04em] mb-1">
                  ...
                  <span className="text-[0.9rem] text-subtle font-normal">
                    {' '}
                    / godz.
                  </span>
                </div>
                <p className="text-[0.82rem] text-subtle mb-6">
                  Wybierz godzinę z kalendarza, aby zarezerwować
                </p>
                ...
                {isAuthenticated ? (
                  ...
                  <>
                    <Link
                      to="/register"
                      className={`block w-full py-3.5 text-center rounded-xl ${GRAD} text-white font-semibold text-sm mb-3 hover:-translate-y-0.5 hover:shadow-[0_10px_35px_rgba(124,58,237,0.45)] transition-all duration-200`}
                    >
                      Zarejestruj się →
                    </Link>
                    <Link
                      to="/login"
                      className="block w-full py-3.5 rounded-xl border border-line-hi text-subtle text-sm font-medium text-center hover:text-white hover:border-white/20 hover:bg-white/[0.04] transition-all duration-200"
                    >
                      Mam już konto
                    </Link>
                  </>
