...
const useRevealGrid = (deps) => {
  ...
  useEffect(() => {
    ...
    if (!root) return;
    root.querySelectorAll('.card-reveal').forEach(el => el.classList.remove('visible'));
    const obs = new IntersectionObserver(
      (entries) => entries.forEach(e => { if (e.isIntersecting) { e.target.classList.add('visible'); obs.unobserve(e.target); } }),
      { threshold: 0.04 }
    );
    const tid = setTimeout(() => root.querySelectorAll('.card-reveal').forEach(el => obs.observe(el)), 40);
...
export const Tutors = () => {
  ...
  const filtered = tutors.filter(t => {
    ...
    const matchFilter = active === 'Wszyscy'; // subject filter requires profile data — future toggle
    ...
    <div className="min-h-screen bg-page text-white pt-[90px]">
      ...
      <section className="py-16 pb-10 border-b border-line">
        <div className="max-w-6xl mx-auto px-8">
          <div className="text-[0.72rem] font-semibold tracking-[0.12em] uppercase text-accent mb-3 animate-fade-in">Platforma korepetycji</div>
          <h1 className="text-[clamp(2rem,5vw,3rem)] font-black tracking-[-0.04em] mb-3 animate-fade-up [animation-delay:0.1s]">
            Znajdź swojego <span className="bg-gradient-to-br from-accent to-accent-2 bg-clip-text text-transparent">korepetytora</span>
          </h1>
          <p className="text-subtle animate-fade-up [animation-delay:0.2s]">
            {loading ? 'Ładowanie...' : `Wybierz spośród ${tutors.length} weryfikowanych specjalistów od INF.03 i INF.04.`}
          </p>
        </div>
      </section>
      ...
      <div className="sticky top-[68px] z-10 bg-page/95 backdrop-blur-xl border-b border-line py-4">
        <div className="max-w-6xl mx-auto px-8 flex flex-wrap items-center gap-4">
          ...
          <span className="ml-auto text-[0.82rem] text-faint">{loading ? '...' : `${filtered.length} wyników`}</span>
        </div>
      </div>
      ...
      <section className="py-16 max-w-6xl mx-auto px-8">
        {error && (
          <div className="mb-8 p-4 bg-red-500/10 border border-red-500/25 rounded-xl text-red-400 text-sm text-center">{error}</div>
        )}

        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(2)].map((_, i) => (
              <div key={i} className="bg-surface border border-line rounded-2xl p-7 h-[320px] animate-pulse">
          ...
          <div className="text-center py-24 text-subtle">
            ...
            <p>{tutors.length === 0 ? 'Brak korepetytorów w systemie.' : 'Brak wyników dla podanej frazy.'}</p>
          </div>
        ) : (
          <div ref={gridRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((tutor, i) => (
              <div key={tutor.id} className="card-reveal" style={{ transitionDelay: `${i * 0.07}s` }}>
                <div className="bg-surface border border-line rounded-2xl p-7 flex flex-col h-full hover:-translate-y-1.5 hover:border-accent/40 hover:shadow-[0_24px_60px_rgba(0,0,0,0.4)] transition-all duration-300">
                  {/* Avatar */}
                  <div className={`w-[68px] h-[68px] rounded-full ${GRAD} flex items-center justify-center text-white text-xl font-black mb-4 border-2 border-accent/30 shrink-0`}>
                    {initials(tutor.name)}
                  </div>

                  <div className="font-bold text-[1.05rem] mb-0.5">{tutor.name}</div>
                  <div className="text-[0.78rem] text-subtle mb-3">{tutor.email}</div>
                  <p className="text-[0.82rem] text-subtle leading-[1.65] mb-4 flex-grow">
                    Korepetytor zarejestrowany w systemie KorkiINF. Sprawdź dostępne terminy na stronie profilu.
                  </p>

                  {/* Joined date */}
                  <div className="flex items-center justify-between pt-4 border-t border-line mb-4">
                    <span className="text-[0.78rem] text-faint">
                      Dołączył {new Date(tutor.created_at).toLocaleDateString('pl-PL', { month: 'long', year: 'numeric' })}
                    </span>
